...
int check_tcp_session(const struct arguments *args, struct ng_session *s,
    ...
    char session[250];
    snprintf(session, sizeof(session), "TCP socket from %s/%u to %s/%u %s socket %d",
...
int get_receive_buffer(const struct ng_session *cur) {
    ...
    int sendbuf = 0;
    socklen_t sendbufsize = (socklen_t) sizeof(sendbuf);
...
void check_tcp_socket(const struct arguments *args,
    ...
    char session[250];
    snprintf(session, sizeof(session), "TCP socket from %s/%u to %s/%u %s loc %u rem %u",
            source, ntohs(s->tcp.source), dest, ntohs(s->tcp.dest),
...
jboolean handle_tcp(const struct arguments *args,
    ...
    snprintf(redirect.raddr, sizeof(redirect.raddr), "%s", args->proxyIp);
    ...
    if (cur == NULL) {
        ...
        if (rport == 443) {
            if (len > 0) {
                snprintf(cur->tcp.hostname, sizeof(cur->tcp.hostname), "%s", hostname);
            } else {
...
int open_tcp_socket(const struct arguments *args,
    ...
    if (version == 4) {
        target_addr = (const struct sockaddr *) &addr4;
        target_addr_len = (socklen_t) sizeof(struct sockaddr_in);
    } else {
        target_addr = (const struct sockaddr *) &addr6;
        target_addr_len = (socklen_t) sizeof(struct sockaddr_in6);
    }
    int err = connect(sock, target_addr, target_addr_len);

...
jboolean handle_udp(const struct arguments *args,
        ...
        if (get_dns_query(args, &cur->udp, data, datalen, &qtype, &qclass, qname) >= 0) {
            ...
            if (0)
                if (check_domain(args, &cur->udp, data, datalen, qclass, qtype, qname)) {
                    ...
                    snprintf(name, sizeof(name), "qtype %d qname %s", qtype, qname);
    ...
    if (rversion == 4) {
        ...
        target_addr_len = (socklen_t) sizeof(addr4);
    } else {
        target_addr = (const struct sockaddr *) &addr6;
        target_addr_len = (socklen_t) sizeof(addr6);
    }

    if (sendto(cur->socket, data, (socklen_t) datalen, MSG_NOSIGNAL,
               target_addr, target_addr_len) != datalen) {

...
jboolean handle_icmp(const struct arguments *args,
    ...
    if (version == 4) {
        target_addr = (const struct sockaddr *) &server4;
        target_addr_len = (socklen_t) sizeof(server4);
    } else {
        target_addr = (const struct sockaddr *) &server6;
        target_addr_len = (socklen_t) sizeof(server6);
    }

    if (sendto(cur->socket, icmp, (socklen_t) icmplen, MSG_NOSIGNAL,
               target_addr, target_addr_len) != icmplen) {
